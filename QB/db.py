...
class DB:
    ...
    def _create_table(self, table_name, field_name, field_type, lenght=None, null=False):
        db_data = {
            table_name: [{'table_field': [{'field_name':field_name, 'field_type':field_type, 'lenght':lenght, 'null': null,
            'commit': 1}]}]
        }
        if os.stat(settings.DBLOGPATH).st_size != 0:
            ...
            if not db_field_name in fields_name:
                json_data[table_name][0]['table_field'].append({'field_name':field_name, 'field_type':field_type, 'lenght':lenght, 'null': null,
                'commit': 1})
            else:
                for x, i in enumerate(json_data[table_name][0]['table_field']):
                    if field_name == json_data[table_name][0]['table_field'][x]['field_name']:
                        ...
                        json_data[table_name][0]['table_field'][x]['null'] = null
...
class IntField(DB):
    def __init__(self, table_name, field_name, lenght, null=False):
        ...
        self.lenght = lenght
        self.null = null
        self._create_table(self.table_name, self.field_name, self.type, self.lenght, self.null)

###############################################

class DbInit(IntField):
    def __init__(self):
